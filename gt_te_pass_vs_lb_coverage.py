...
# Defense: Opponent LBs with > 50 snaps (coverage grade)
...
# -------------- Helper: opponent LB coverage room --------------
def opponent_lb_room_coverage(df_opp: pd.DataFrame):
    """
    Returns a tuple:
      (room_avg_coverage_rounded, dataframe_of_all_LBs_sorted_by_snaps_with_rounded_grades)
    """
    if df_opp.empty:
        return (np.nan, df_opp)

    w = df_opp["snap_counts_defense"].clip(lower=0).fillna(0)
    v_cov = df_opp["grades_coverage_defense"]
    room_avg_coverage = (v_cov.mul(w)).sum() / (w.sum() if w.sum() > 0 else len(df_opp))
    room_avg_coverage = round(float(room_avg_coverage), 2)

    lbs_sorted = df_opp.sort_values("snap_counts_defense", ascending=False).copy()
    lbs_sorted["grades_coverage_defense"] = lbs_sorted["grades_coverage_defense"].round(2)
    lbs_sorted["snap_counts_defense"] = lbs_sorted["snap_counts_defense"].astype(int)
    return (room_avg_coverage, lbs_sorted)

# Precompute per-opponent LB room + list
opp_lb_summaries = {}
for opp in OPPONENTS:
    opp_df = opp_lbs[opp_lbs["team_name"] == opp]
    opp_lb_summaries[opp] = opponent_lb_room_coverage(opp_df)

# -------------- Build formatted output + CSV --------------
rows = []  # for CSV

print("=" * 80)
print("GT TE PASS (grades_pass_route) vs OPP LB COVERAGE (grades_coverage_defense) MATCHUPS")
print("=" * 80)

for opp in OPPONENTS:
    lb_cov_avg, lbs_sorted = opp_lb_summaries.get(opp, (np.nan, pd.DataFrame()))

    print(f"\n{OUR_TEAM} TEs vs {opp} Linebackers (Coverage)")  # section header

    for _, r in gt_tes.iterrows():
        player = r["player"]
        te_pass = round(float(r["grades_pass_route"]), 2) if pd.notna(r["grades_pass_route"]) else np.nan

        # Team-level matchup (TE pass vs LB room coverage avg)
        delta_pass_cov = round(te_pass - lb_cov_avg, 2) if pd.notna(te_pass) and pd.notna(lb_cov_avg) else np.nan

        print(f"{player} vs {opp} Defense — TE Pass: {te_pass:.2f} vs LB Avg Coverage: {lb_cov_avg if pd.notna(lb_cov_avg) else float('nan'):.2f} (Δ {delta_pass_cov if pd.notna(delta_pass_cov) else float('nan'):.2f})")
        ...
        rows.append({
            ...
            "opp_lb_coverage_grade": lb_cov_avg,
            "opp_lb_name": None,
            "opp_lb_snaps": None,
            "delta_pass_coverage": delta_pass_cov
        })
        ...
        if not lbs_sorted.empty:
            for rank, (_, lb) in enumerate(lbs_sorted.iterrows(), start=1):
                ...
                lb_cov_grade = float(lb["grades_coverage_defense"]) if pd.notna(lb["grades_coverage_defense"]) else np.nan
                lb_snaps = int(lb["snap_counts_defense"]) if pd.notna(lb["snap_counts_defense"]) else 0
                delta_pass_cov_lb = round(te_pass - lb_cov_grade, 2) if pd.notna(te_pass) and pd.notna(lb_cov_grade) else np.nan

                print(f"{player} vs {opp} Defense (LB #{rank} by snaps — {lb_name}) — TE Pass: {te_pass:.2f} vs LB Coverage: {lb_cov_grade if pd.notna(lb_cov_grade) else float('nan'):.2f} (snaps {lb_snaps}, Δ {delta_pass_cov_lb if pd.notna(delta_pass_cov_lb) else float('nan'):.2f})")
                ...
                rows.append({
                    ...
                    "player_position": "TE",
                    "player_pass_grade": te_pass,
                    "opp_lb_coverage_grade": lb_cov_grade,
                    "opp_lb_name": lb_name,
                    "opp_lb_snaps": lb_snaps,
                    "delta_pass_coverage": delta_pass_cov_lb
                })

# Save a tidy CSV with every printed row
out_df = pd.DataFrame(rows)
out_df.to_csv("gt_te_pass_vs_lb_coverage_breakdown.csv", index=False)
print(f"\nSaved -> gt_te_pass_vs_lb_coverage_breakdown.csv")
print(f"Total TE vs LB (coverage) matchups analyzed: {len(rows)}")
